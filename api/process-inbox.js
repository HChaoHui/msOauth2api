...
module.exports = async (ctx, next) => {
    ...
    try {
        ...
        imap.once('ready', async () => {
            try {
                ...
                if (results.length === 0) {
                    ...
                    imap.end();
                    return response.error(ctx, 'No Inbox emails found.', 404);
                }
                ...
                f.once('error', (err) => {
                    ...
                    imap.end();
                    return response.error(ctx, { error: 'Fetch error', details: err.message });
                });
                ...
                f.once('end', async () => {
                    ...
                    imap.end();
                    return response.success(ctx, { message: 'Emails processed successfully.' });
                });
            } catch (err) {
                console.error('Error:', err);
                imap.end();
                return response.error(ctx, { error: 'Error processing emails', details: err.message });
            }
        });
        ...
        imap.once('error', (err) => {
            console.log(err);
            imap.end();
            return response.error(ctx, { error: 'IMAP connection error', details: err.message });
        });
        ...
        return response.error(ctx, { error: 'Error', details: error.message });
    }
};