...
async function get_access_token(refresh_token, client_id) {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse JSON: ${parseError.message}, response: ${responseText}`
    );
  }
}
...
const getNewMail = (email, authString, mailbox, response_type) => {
          ...
          msg.on("body", (stream, info) => {
            simpleParser(stream, (err, mail) => {
              ...
              if (response_type === "json") {
                ...
                reject(
                  new Error('Invalid response_type. Use "json" or "html".')
                );
              }
            });
          });
...
async function get_emails(access_token, mailbox, params) {
  if (!access_token) {
  ...
  try {
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/me/mailFolders/${mailbox}/messages?$top=1&$orderby=receivedDateTime desc`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
    ...
    if (!response.ok) {
      try {
        const access_token = await get_access_token(params.refresh_token, params.client_id);
        const authString = generateAuthString(params.email, access_token);

        const mail_data = await getNewMail(
          params.email,
          authString,
          params.mailbox,
...
module.exports = async (ctx, next) => {
  ...
  if (!refresh_token || !client_id || !email || !mailbox) {
    return response.error(
      ...
      {
        error:
          "Missing required parameters: refresh_token, client_id, email, or mailbox",
      },
      400
    );
  }
  ...
  try {
    ...
    if (graph_api_result.access_token) {
      ...
      const result = await get_emails(
        graph_api_result.access_token,
        temp_mailbox,
        { refresh_token, client_id, email, mailbox, response_type }
      );
    ...
    response.error(ctx, { error: error.message }, 500);
  }
};
