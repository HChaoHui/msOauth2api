...
async function get_emails(access_token, mailbox, params) {
  ...
  try {
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/me/mailFolders/${mailbox}/messages?$top=10000`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
    ...
    if (!response.ok) {
      try {
        ...
        const mail_data = await getAllMail(
          params.email,
          authString,
          params.mailbox
        );
...
module.exports = async (ctx, next) => {
  ...
  let { refresh_token, client_id, email, mailbox } =
  ...
  try {
    ...
    if (graph_api_result.access_token) {
      ...
      const result = await get_emails(
        ...
        temp_mailbox,
        { refresh_token, client_id, email, mailbox }
      );
